...
public class HttpUtil {
  ...
  public static CloseableHttpClient getHttpClient(String accountName, Properties proxyProperties) {
    ...
    System.out.println("DEBUGSNOWFLAKEPROXY: From Snowflake Ingest HttpUtil. Created HttpClientSettingsKey: " + key);

    return httpClientCache.computeIfAbsent(key, k -> {
      LOGGER.info("Creating new HTTP client for key: {}", k);
  ...
  private static HttpClientSettingsKey createHttpClientSettingsKey(String accountName, Properties proxyProperties) {
    System.out.println("Creating HttpClientSettingsKey for account: " + accountName);

    // TODO: for debugging purpose, remove before merging
    if (proxyProperties == null) {
      System.out.println("DEBUGSNOWFLAKEPROXY: From Snowflake Ingest HttpUtil. No proxy properties provided.");
    }
    // TODO: for debugging purpose, remove before merging
    if (proxyProperties != null) {
      Boolean useProxy = Boolean.parseBoolean(proxyProperties.getProperty(SFSessionProperty.USE_PROXY.getPropertyKey(), "false"));
      System.out.println("DEBUGSNOWFLAKEPROXY: From Snowflake Ingest HttpUtil. Use Proxy: " + useProxy);
    }


    if (proxyProperties != null && proxyProperties.containsKey(SFSessionProperty.USE_PROXY.getPropertyKey())) {

      System.out.println("DEBUGSNOWFLAKEPROXY: From Snowflake Ingest HttpUtil. Proxy Properties found.");
      ...
      if (useProxy) {
        ...
        System.out.println("DEBUGSNOWFLAKEPROXY: From Snowflake Ingest HttpUtil. Proxy Host: " + proxyHost);
        System.out.println("DEBUGSNOWFLAKEPROXY: From Snowflake Ingest HttpUtil. Proxy Port: " + proxyPortStr);
        int proxyPort = 0;
        try {
        ...
        System.out.println("DEBUGSNOWFLAKEPROXY: From Snowflake Ingest HttpUtil. Non Proxy Hosts: " + nonProxyHosts);
        System.out.println("DEBUGSNOWFLAKEPROXY: From Snowflake Ingest HttpUtil. Proxy User: " + proxyUser);
    ...
    if ("true".equalsIgnoreCase(System.getProperty(USE_PROXY)) && !shouldBypassProxy(accountName)) {
      System.out.println("DEBUGSNOWFLAKEPROXY: From Snowflake Ingest HttpUtil. Using system properties for proxy configuration.");
